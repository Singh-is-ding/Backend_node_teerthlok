...
export default function VRPlayer({ videoUrl, projection = "360", title = "" }) {
  ...
  useEffect(() => {
    ...
    function render() {
      // Only use spherical lookAt in drag mode.
      // When gyro is active, camera.quaternion is set directly by the handler.
  ...
  const enableGyro = useCallback(async () => {
    ...
    try {
      ...
      // Pre-allocate — never allocate inside the hot event handler
      const _euler   = new THREE.Euler();
      const _qDevice = new THREE.Quaternion();
      const _qScreen = new THREE.Quaternion();
      // Rotate world frame: device Z-up → camera Y-up (-90° around X)
      const _qWorld  = new THREE.Quaternion(-Math.sqrt(0.5), 0, 0, Math.sqrt(0.5));
      ...
      const handler = (e) => {
        ...
        // Screen rotation offset (portrait=0, landscape=90, etc.) in radians
        const screenAngle = THREE.MathUtils.degToRad(
          window.screen?.orientation?.angle ?? window.orientation ?? 0
        );

        // YXZ Euler order = yaw first, then pitch, then roll — correct for head-tracking
        _euler.set(
        ...
        // Apply world-frame correction (device coords → camera coords)
        ...
        // Drive the camera quaternion directly — no spherical middleman
        if (cameraRef.current) {
          cameraRef.current.quaternion.copy(_qDevice);
        }

        // Keep sphericalRef in sync so drag works after gyro is disabled
        _fwd.set(0, 0, -1).applyQuaternion(_qDevice);
        sphericalRef.current = {
          theta: Math.atan2(_fwd.x, _fwd.z),
          phi: Math.max(0.05, Math.min(Math.PI - 0.05,
            Math.acos(Math.max(-1, Math.min(1, _fwd.y)))
          )),
        };
      };
